...
def calc_distance(time_pressed, remaining_time, total_time, highest_distance):
    ...
    result = calc_distance(time_pressed+1, remaining_time -
                           1, total_time, highest_distance)
...
def calc_distance_4(remaining_time, total_time, highest_distance):
...
def main1(inputfile, expected_result_1=0, expected_result_2=0):
    ...
    for race in race_info:
        result = calc_distance(
            0, race_info[race]['time'], race_info[race]['time'], race_info[race]['distance'])
...
def main1_2(inputfile, expected_result_1=0, expected_result_2=0):
    ...
    margine_for_error = []
    for race in race_info:
        result = calc_distance_3(
            race_info[race]['time'], race_info[race]['time'], race_info[race]['distance'])
...
def main2_2(inputfile, expected_result_1=0, expected_result_2=0):
    race_info = read_races_2(read_file(inputfile))
    for race in race_info:
        result = calc_distance_4(
            race_info[race]['time'], race_info[race]['time'], race_info[race]['distance'])
    actual_result = result
    print("="*12)
    print(f"Running in main2_2")
